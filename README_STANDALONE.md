...
### v3.0.9 mobile Stage UI note

The mobile Stage build toolbar was compacted so `Блок построения`, `Настил`, `Лестница`, and `Очистить` fit in one horizontal row without broken button labels. This is a mobile-only visual hotfix; desktop behavior and calculations are unchanged.
